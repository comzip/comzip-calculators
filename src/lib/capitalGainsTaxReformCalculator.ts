 ...
 *
 * ⚠️ 2026년 값(20%/30%)은 **중과 유예가 2026-05-09에 일몰된 사실에 의존**한다.
 * 2022-05-10 ~ 2026-05-09 사이 양도분은 한시적으로 중과가 배제됐고 장기보유
 * 특별공제도 정상 적용됐으므로, 그 기간의 양도를 이 표로 계산하면 세액이 크게
 * 과대계상된다(실측 +82%). 이 계산기는 양도"일"을 입력받지 않고 연도만 받으므로
 * **2026-05-10 이후 양도분 기준**으로만 맞다 — 페이지에도 같은 취지를 명시했다.
 * 유예가 재도입되면 2026년 값을 0으로 바꿔야 한다.
 */
